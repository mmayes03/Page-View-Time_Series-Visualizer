...
df = pd.read_csv('Files/fcc-forum-pageviews.csv', parse_dates= ['date'], index_col= 'date')
...
def draw_box_plot():
    ...
    df_box = df_box.sort_values(by = ["month_num"]) #Sort dates

    fig, axes = plt.subplots(nrows= 1, ncols=2, figsize = (10,5))
    fig.tight_layout(pad=5.0)
    axes[0] = sb.boxplot(x=df_box['year'], y=df_box['value'], ax=axes[0])
    axes[1] = sb.boxplot(x=df_box['month'], y=df_box['value'], ax=axes[1])
